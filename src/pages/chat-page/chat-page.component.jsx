...
class ChatPage extends Component {
    ...
    componentDidMount = () => {
        ...
        fetch( '/users', requestOptions)
            .then(async response => {
                ...
                socket = io(URL.socketURL, { query: `chatID=${currentUser.user.id}&token=${currentUser.tokens.access.token}` });
    ...
    handleChatSwitch = userId => {
        ...
        fetch( `/messages?userId=${currentUser.user.id}&toId=${userId}`, requestOptions)
            .then(async response => {
                const data = await response.json();
