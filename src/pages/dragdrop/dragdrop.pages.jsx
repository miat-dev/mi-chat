...
class DragDropPage extends Component {
    ...
    componentDidMount = () => {
        ...
        socket = io(URL.socketURL, { query: `roomName=${roomName}&token=${currentUser.tokens.access.token}` });
        ...
        fetch( `/rooms/manage/${roomId}`, { headers })
            .then(async response => {
    ...
    onDrop = (ev, cat) => {
        ...
        fetch( `/rooms/manage/${roomId}`, requestOptions)
            .then(async response => {
                const data = await response.json();
